...
const LoginPage = () => {
  ...
  const [loginStudent, { isLoading: isStudentLoading }] =
    useLoginStudentMutation();
  ...
  const submitHandler = async (data) => {
    ...
    try {
      ...
      const encryptedUser = encrypt(
        isStudentLogin ? response?.student : response?.admin
      );
      sessionStorage.setItem(
        appConfig.sessionKeys?.token,
        encryptedToken?.encrypted
      );
      sessionStorage.setItem(
        appConfig?.sessionKeys?.user,
  ...
  useEffect(() => {
    if (isAuthenticated) {
      return navigate(
        isStudentLogin ? `${TabValues?.requests?.to}` : "/history"
      );
    }
  }, [isAuthenticated]);
  return (
      ...
      <Slide in={showLogin} direction="up">
        <Card className="login-page__login-form">
          ...
          <TextField
            ...
            helperText={
              errors?.[passwordField] ? errors?.[passwordField]?.message : " "
            }
            fullWidth
            variant="standard"
            slotProps={{
          ...
          <Button
            ...
            startIcon={
              isAdminLoading || isStudentLoading ? (
          ...
          >
            {isStudentLogin
              ...
              : appConfig.buttonLabels.adminLogin}
          </Button>
          <Typography variant="body2" className="login-page__change-user">
            {isStudentLogin
              ? appConfig.captions.notAStudent
              : appConfig.captions.notAnAdmin}{" "}
            <span
              className="login-page__change-user--link"
